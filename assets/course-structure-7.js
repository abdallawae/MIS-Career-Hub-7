(()=>{
  ...
  ['sql','excel','power-bi','systems-analysis','erp'].forEach(k=>{if(curated[k])window.MIS_CURRICULUM_20[k]=normalize(curated[k]);});
  ['sql','excel','power-bi','systems-analysis','erp'].forEach(k=>{const c=window.MIS_CURRICULUM_20[k],bank=window.MIS_RICH_QUIZZES?.[k];if(c&&bank){let n=1;c.levels.forEach(level=>level.forEach(lesson=>{lesson[6]=bank[n++]||[]}));}});
  window.MIS_COURSE_CONTENT_7={...(window.MIS_COURSE_CONTENT||{})};['sql','excel','power-bi','systems-analysis','erp'].forEach(k=>{if(window.MIS_CURRICULUM_20[k])window.MIS_COURSE_CONTENT_7[k]=window.MIS_CURRICULUM_20[k];});
