-- MIS Career Hub — repair migration for the live 20-lesson schema.
-- This fixes installations where the progress/project tables existed before course_slug was added.
-- Safe to run more than once.

-- 1) Make the course_slug columns used by the current web app available.
alter table public.course_project_submissions add column if not exists course_slug text;
alter table public.lesson_progress add column if not exists course_slug text;
alter table public.quiz_results add column if not exists course_slug text;
alter table public.course_enrollments add column if not exists course_slug text;
alter table public.certificates add column if not exists course_slug text;

-- 2) Keep existing rows valid. New submissions/progress written by the app will carry the real slug.
update public.course_project_submissions set course_slug = coalesce(course_slug, 'unknown') where course_slug is null;
update public.lesson_progress set course_slug = coalesce(course_slug, 'unknown') where course_slug is null;
update public.quiz_results set course_slug = coalesce(course_slug, 'unknown') where course_slug is null;
update public.course_enrollments set course_slug = coalesce(course_slug, 'unknown') where course_slug is null;
update public.certificates set course_slug = coalesce(course_slug, 'unknown') where course_slug is null;

alter table public.course_project_submissions alter column course_slug set default 'unknown';
alter table public.lesson_progress alter column course_slug set default 'unknown';
alter table public.quiz_results alter column course_slug set default 'unknown';
alter table public.course_enrollments alter column course_slug set default 'unknown';
alter table public.certificates alter column course_slug set default 'unknown';

alter table public.course_project_submissions alter column course_slug set not null;
alter table public.lesson_progress alter column course_slug set not null;
alter table public.quiz_results alter column course_slug set not null;
alter table public.course_enrollments alter column course_slug set not null;
alter table public.certificates alter column course_slug set not null;

-- 3) Unique constraints required by the browser upserts and final-project RPC.
create unique index if not exists uq_project_submission_user_course
...
create unique index if not exists uq_lesson_progress_user_course_level_lesson
  on public.lesson_progress(user_id, course_slug, level_number, lesson_number);

-- 4) Safe indexes. No assumptions beyond columns used by the current app/RLS.
create index if not exists idx_project_submissions_user
  on public.course_project_submissions(user_id);
...
-- 6) Owner/student RLS for the tables used by the current app.
...
create policy "project_select_own_or_admin" on public.course_project_submissions for select to authenticated
  using ((select auth.uid()) = user_id or (select private.is_admin()));
create policy "project_insert_own" on public.course_project_submissions for insert to authenticated
  with check ((select auth.uid()) = user_id);
create policy "project_update_own_or_admin" on public.course_project_submissions for update to authenticated
  using ((select auth.uid()) = user_id or (select private.is_admin()))
  with check ((select auth.uid()) = user_id or (select private.is_admin()));
create policy "project_delete_admin" on public.course_project_submissions for delete to authenticated
...
create policy "lesson_progress_insert_own" on public.lesson_progress for insert to authenticated
  with check ((select auth.uid()) = user_id);
create policy "lesson_progress_update_own_or_admin" on public.lesson_progress for update to authenticated
  using ((select auth.uid()) = user_id or (select private.is_admin()))
  with check ((select auth.uid()) = user_id or (select private.is_admin()));
...
create policy "quiz_results_select_own_or_admin" on public.quiz_results for select to authenticated
  using ((select auth.uid()) = user_id or (select private.is_admin()));
create policy "quiz_results_insert_own" on public.quiz_results for insert to authenticated
  with check ((select auth.uid()) = user_id);
...
create policy "enrollments_select_own_or_admin" on public.course_enrollments for select to authenticated
  using ((select auth.uid()) = user_id or (select private.is_admin()));
...
create policy "lesson_content_read_published" on public.lesson_content for select to authenticated
  using (published = true or (select private.is_admin()));
create policy "lesson_content_admin_all" on public.lesson_content for all to authenticated
...
create policy "lesson_quizzes_read_published" on public.lesson_quizzes for select to authenticated
  using (published = true or (select private.is_admin()));
create policy "lesson_quizzes_admin_all" on public.lesson_quizzes for all to authenticated
  using ((select private.is_admin())) with check ((select private.is_admin()));
