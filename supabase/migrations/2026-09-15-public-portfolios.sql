...
-- Public visitors can only see profiles explicitly published by the owner.
...
-- A student can manage only their own portfolio.
...
-- Create a safe default slug from the authenticated user's UUID.
-- Students can later edit the slug from their Portfolio settings.
...
begin
  ...
  insert into public.portfolio_profiles(user_id, public_slug)
